...
function SidebarProvider({
  ...
  const [_openRight, _setOpenRight] = React.useState(defaultOpen);
  ...
  // Toggle function for both sidebars
  const toggleSidebar = React.useCallback(
    ...
    [isMobile, setOpenLeft, setOpenRight, setOpenMobileLeft, setOpenMobileRight]
  );
  ...
  const contextValue = React.useMemo<SidebarContextProps>(
    () => ({
      left: { state: leftState, open: openLeft, setOpen: setOpenLeft },
      right: { state: rightState, open: openRight, setOpen: setOpenRight },
      isMobile,
      openMobileLeft,
      openMobileRight,
      setOpenMobileLeft,
...
function Sidebar({
  ...
  // Select the appropriate state and mobile controls based on the side
  const sidebarState = side === "left" ? left.state : right.state;
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={cn(
          "fixed inset-y-0 z-10 h-svh w-[var(--sidebar-width)] transition-[left,right,width] duration-200 ease-linear",
          side === "left"
...
function SidebarTrigger({
  side = "left", // Default to left sidebar
  className,
  onClick,
  children,
  ...props
  ...
  return (
    <Button
      ...
      size="icon"
      className={cn(
        "size-8 border-2 border-neutral-250 cursor-pointer",
        className
      )}
      onClick={(event) => {
        onClick?.(event);
        toggleSidebar(side); // Pass the side parameter
      }}
      {...props}
    >
      {children}
      {/* <Menu className="text-neutral-250" /> */}
      {/* <span className=" text-black">T {side} S</span> */}
    </Button>
  );
}
...
function SidebarMenuButton({
  ...
  variant = "default",
  size = "default",
  side = "left", // New prop to specify which sidebar
  tooltip,
  className,
  ...props
  ...
  const Comp = asChild ? Slot : "button";
  const { isMobile, left, right } = useSidebar();

  // Select the appropriate state based on the side
  const sidebarState = side === "left" ? left.state : right.state;
  ...
  return (
    <Tooltip>
      ...
      <TooltipContent
        side="right"
        align="center"
        hidden={sidebarState !== "collapsed" || isMobile} // Use sidebar-specific state
        {...tooltip}
      />
    </Tooltip>
  );
