...
import { defaultUserProfile, getTotalComments, sliceMethod } from "@/lib/utils";
  ...
  const commentsData =
    post?.postInfo
      ...
      ?.flatMap((item: any) => {
        ...
        return item.comments.map((subComment: any) => ({
          id: subComment.id,
          username: user?.username || user?.name || "Unknown",
          handle: sliceMethod(user?.wallet_address),
          time: "", // Add logic for time if available
          content: subComment.comment,
          avatarSrc: user?.avatar || defaultUserProfile,
          likes: 0,
          replies: 0, // No nested replies in provided data
          shares: 0,
          diamonds: 0,
          views: 0,
          follow: 0,
